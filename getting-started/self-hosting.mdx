...
Once the topic is created, you will need to create a subscription to the topic. Plunk requires an HTTPS subscription to the topic on the `/api/webhooks/incoming/sns` endpoint.

<Info>
    Replace `https://plunk.example.com` with the domain where you want to host Plunk.
</Info>
...
#### 3.1 Confirming the SNS Topic
Before an SNS topic can be used, it must be confirmed.

Head over to the [SNS console](https://console.aws.amazon.com/sns/v3/home) and select the subscription created in step 1.1.
